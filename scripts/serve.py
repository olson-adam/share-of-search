...
def export(snapshot: dict, out: Path) -> None:
    ...
    # inline the single css + single js that vite emits, then embed the data
    for m in re.finditer(r'<link rel="stylesheet"[^>]*href="/(assets/[^"]+\.css)"[^>]*>', html):
        css = (DIST / m.group(1)).read_text(encoding="utf-8")
        html = html.replace(m.group(0), f"<style>{css}</style>")
    for m in re.finditer(r'<script type="module"[^>]*src="/(assets/[^"]+\.js)"[^>]*></script>', html):
